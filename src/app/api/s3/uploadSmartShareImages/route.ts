...
import { getPresignedUrl, getFileUrl } from "@/lib/aws";
...
export async function POST(req: NextRequest) {
  try {
    const { files, eventId, eventName } = await req.json();
    ...
    const bucket = process.env.NEXT_AWS_BUCKET_SMART_SHARE_NAME!;

    const presignedImagesMetadata = await Promise.all(
      files.map(async (file) => {
        // 1) build your key
        const folder = `${userId}/${eventName}`;
