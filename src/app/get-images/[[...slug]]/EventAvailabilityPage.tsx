...
function EventAvailabilityPage({
  eventData,
}: {
  eventData:
    | Omit<
        SmartShareEventsDataInterface,
        "description" | "total_size" | "created_at"
      >
    | undefined;
}) {
  return (
    <Card className="max-w-sm md:max-w-3xl mx-auto overflow-hidden rounded-2xl shadow-lg bg-primary-foreground">
      ...
      <div className="relative w-full h-60 md:h-96">
        <Image
          src={
            eventData?.cover_image instanceof File
              ? AltCoverImage
              : eventData?.cover_image || AltCoverImage
          }
          layout="fill"
          objectFit="cover"
          alt="event-image"
        ...
        <div className="flex items-center justify-between">
          <div>
            <Label className="text-sm md:text-base font-semibold">
              Get your images by face recognition at{" "}
            </Label>
            <Label className="text-base md:text-xl font-bold text-primary">
              {eventData?.name.charAt(0).toUpperCase() +
                (eventData?.name.slice(1) || "")}
            </Label>
          </div>
          <Popover>
            ...
            <PopoverContent className="w-80 p-4 text-sm text-muted-foreground bg-primary-foreground">
              ...
              algorithms detect your face in every photo captured with 99.2%
              accuracy. Just fill this form, capture a selfie, and we’ll send
              all your photos from <strong>{eventData?.name}</strong> directly
              to you—automatically and privately!
              <br />
              <br />
