...
export default function PricingSectionCards({data}:{data:BirthdayEventPriceType[]}) {
  return (
    <>
      {/* Pricing */}
      <div className="container space-y-8 sm:space-y-20">
        ...
        {data.map((items, index)=>(
            <Card key={index} className={`h-full relative overflow-hidden ${index===1?"border-2 border-primary scale-100 sm:scale-105 shadow-none sm:shadow-xl shadow-primary":""}`}>
                {/* Background Image */}
                <Image
                ...
                <CardHeader className="text-center pt-16">
                    ...
                    <CardTitle className="font-bold text-4xl">{items.heading}</CardTitle>
                    <span className="text-muted-foreground">{items.description}</span>
                </CardHeader>
                <CardContent>
                    {
